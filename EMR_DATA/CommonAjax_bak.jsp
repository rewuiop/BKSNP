...
/*                          
							+" select distinct U.USERNAME AS UNAME, R.DEPARTMENT AS CLINCODE, Substring(R.RECORD_TIME,1,4)+Substring(R.RECORD_TIME,6,2)+Substring(R.RECORD_TIME,9,2) +' '+U.USERNAME AS LEVEL2 ,count(R.CODE) AS CNT, Substring(R.RECORD_TIME,1,4)+Substring(R.RECORD_TIME,6,2)+Substring(R.RECORD_TIME,9,2) AS INDATE "
							+" , DO.NAME,'0' AS ORDERBY ,R.DOC_CODE ,R.CODE,CONVERT(VARCHAR,R.SEQ_NO) ,'1' SDECHECK,  R.MODIFY_TIME,R.TEMPSAVE,R.CHOS_NO "
							+" from BERDMRECD R "
							+" left join BERDMDOCU DO on R.DOC_CODE = DO.CODE "
							+" inner join ETL_PICUSERM U on R.MODIFIER_CODE = U.USERPIDN  and U.USERDEPT=R.DEPARTMENT "
							+" where R.PTID= ? and R.DEPARTMENT= ?   and R.STATUS='0' "
							+" group by R.DEPARTMENT+'_'+R.CHOS_NO+'_'+R.DOC_CODE+'_'+Substring(R.RECORD_TIME,1,4)+Substring(R.RECORD_TIME,6,2)+Substring(R.RECORD_TIME,9,2)+R.CODE ,U.USERNAME "
							+" ,R.DEPARTMENT+'_'+R.CHOS_NO,DO.NAME,R.CHOS_NO,R.DEPARTMENT "
*/
...
/*
							...
							+" left join BERDMDOCU DO on R.DOC_CODE = DO.CODE "
							+" inner join ETL_PICUSERM U on R.MODIFIER_CODE = U.USERPIDN  and U.USERDEPT=R.DEPARTMENT "
							+" where R.PTID= ? and R.DEPARTMENT= ?   and R.STATUS='0'  and RECORD_TIME=? "
							+" group by R.DEPARTMENT+'_'+R.CHOS_NO+'_'+R.DOC_CODE+'_'+Substring(R.RECORD_TIME,1,4)+Substring(R.RECORD_TIME,6,2)+Substring(R.RECORD_TIME,9,2)+R.CODE ,U.USERNAME "
							+" ,R.DEPARTMENT+'_'+R.CHOS_NO,DO.NAME,R.CHOS_NO,R.DEPARTMENT "
*/
...
//							+" group by X.INDATE,X.INDATE+'_'+X.UNAME "
		...
		else if(fname.equals("getScanAllImagePath"))
		{
			String sqlStr="SELECT  T.PATID, "
						       ...
						       +"         AND T.PATID = ?  "
						       +"         AND CONVERT(varchar,T.TREATNO) = ? "
						       +"          AND F.FORMCODE=C.FORMCODE  ORDER BY F.ORDERBY,F.FORMCODE,T.INDATE desc ";
		...
		{
			String sqlStr="update BERDMPRVT set FST_DOC_ID = ? , SND_DOC_ID = ? where DOCTOR_ID = ? and DIVISION_ID = ? and GUBUN = ? ";
			...
			//String pagenum=getRequestValue(res,i,1,"pagenum");
			
			String[][] param1=new String[1][5];
			...
			param1[0][4]=gubun;
			/*
			if(!pagenum.equals("")&&!doctorId.equals("0")){
				String sqlStr1="update BERDMPRVT set MONITOR = ? where DOCTOR_ID = ? ";
				String[][] param2=new String[1][2];
				param2[0][0]=pagenum;
				param2[0][1]=doctorId; 
				DoSqlUpdate_PS(sqlStr1,param2);
			}
			*/
		...
		{
			String sqlStr="insert into BERDMPRVT(DOCTOR_ID,DIVISION_ID,FST_DOC_ID,SND_DOC_ID,GUBUN) values(?,?,?,?,?) ";
		...
		{
			String sqlStr=" select USERID from BERDMAUTH where USERID=? and ACCESSYN='1' and OPENTYPE=? ";
